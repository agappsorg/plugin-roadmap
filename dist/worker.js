...
// src/worker.ts
var ROADMAP_STATE_KEY = { scopeKind: "instance", stateKey: "roadmap-items" };
var plugin = definePlugin({
  async setup(ctx) {
    ctx.data.register("roadmap", async () => {
      const items = await ctx.state.get(ROADMAP_STATE_KEY);
      return items ?? [];
    });
    ...
    ctx.actions.register("move-item", async (params) => {
      ...
      items[idx] = { ...items[idx], column, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
      await ctx.state.set(ROADMAP_STATE_KEY, items);
      return { ok: true };
    });
  },
